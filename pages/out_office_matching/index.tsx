...
const ProgramPage: NextPage = () => {
	...
	const gsicExpertController = new DefaultController('GsicExpert');
	const gsicMatchingExpertController = new DefaultController(
		'GsicMatchingExpert'
	);
	...
	const fetchMatchingList = async () => {
		setIsLoading(true);
		try {
			gsicExpertController.findAllItems({}, (res) => {
				const filteredList = res.data.result.rows.filter((item: any) =>
					JSON.parse(item.FIELD).some((field) =>
	...
	const fetchAppliedMatchingList = async () => {
		...
		setIsLoading(true);
		try {
			gsicMatchingExpertController.findAllItems(
				{
					GSIC_MEMBER_IDENTIFICATION_CODE: id || 0,
				},
				(res) => {
	...
	const handleApplyClick = (matching: any) => {
		setSelectedMatching(matching);
	...
	const handleApplyConfirm = async () => {
		gsicMatchingExpertController.createItem(
			{
				GSIC_MEMBER_IDENTIFICATION_CODE: id,
				GSIC_EXPERT_IDENTIFICATION_CODE:
					selectedMatching.GSIC_EXPERT_IDENTIFICATION_CODE,
			},
			(res) => {
	...
	const isMatchingApplied = (matchingId: string) => {
		return completedList.some(
			(completed) =>
				completed.GSIC_MATCHING_EXPERT_IDENTIFICATION_CODE ===
				matchingId
		);
	};
	...
	return (
		...
		>
							...
							<Box sx={{ py: 1 }}>
								{preparingList.map((item, index) => {
									const isApplied = isMatchingApplied(
										item.GSIC_EXPERT_IDENTIFICATION_CODE
									);
									return (
										<Card
											key={index}
							...
							<Box sx={{ py: 1 }}>
								{completedList.map((item, index) => (
									<Card
										...
										onClick={() =>
											handleMatchingClick(item.GsicExpert)
										}
									>
										<Box sx={{ p: 2 }}>
											<Typography
					...
					{selectedMatching && (
						<Box sx={{ mt: 1 }}>
							...
							<Box sx={{ mb: 3 }}>
								...
								>
									{JSON.parse(selectedMatching.FIELD).map(
							...
							<Box sx={{ mb: 3 }}>
								...
								>
									{selectedMatching.CONSULTING_FEE}
								</Typography>
							</Box>
			...
			>
				...
				<DialogContent>
					<Typography>
						{selectedMatching?.NAME} 전문가와 매칭을
						신청하시겠습니까?
					</Typography>
				</DialogContent>
				<DialogActions>
