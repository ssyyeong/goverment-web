...
const Page: NextPage = () => {
	...
	//* Controller
	const insightController = new DefaultController('InsightEn');
	const insightCategoryController = new DefaultController(
		'InsightCategoryEn'
	);
	...
	const getInsight = () => {
		insightController.findAllItems(
			{
				INSIGHT_CATEGORY_EN_IDENTIFICATION_CODE:
					...
					return (
						<Typography
							...
							sx={{
								...
								borderColor:
									tab ===
									item.INSIGHT_CATEGORY_EN_IDENTIFICATION_CODE
										? 'primary.main'
										: '#c8c8c8',
								color:
									tab ===
					...
					{insightDataList?.map((item, index) => {
						return (
							<Box
								...
								onClick={() => {
									router.push(
										`/en/insight/${item.INSIGHT_EN_IDENTIFICATION_CODE}`
									);
								}}
							>
								<Box
