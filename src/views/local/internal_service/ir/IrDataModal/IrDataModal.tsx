...
import { IUserIRData } from '../../../../../@types/model';
import { IInvestInfoType } from '../../../../../../pages/internal_service/ir/management';
...
const IrDataModal = (props: IIrDataModalProps) => {
	//* Controller
	const irApplicationHistoryController = new DefaultController(
		'IrApplicationHistory'
	);
	//* Constants
	// 투자 라운드
	 ...
	 * ir 히스토리 데이터 가져오기
	 */
	const getIrHistoryData = () => {
		irApplicationHistoryController.getOneItemByKey(
			{
	...
	const updateUserIrInfo = () => {
		...
		irApplicationHistoryController.updateItem(
			{
				...userIrInfo,
	...
	React.useEffect(() => {
		if (props.modalOpen && props.irApplicationId) {
			getIrHistoryData();
		}
	}, [props.modalOpen, props.irApplicationId]);
	...
	return (
		...
		>
			...
			>
					...
					{isEdit ? (
						...
						>
							...
							<DownloadForOfflineIcon
								onClick={() => {
									window.open(irDeckFile.FILE_URL, '_blank');
								}}
								sx={{
									cursor: 'pointer',
								}}
				...
				{/* 기업 정보 */}
				<Box mt={2}>
					<Typography fontWeight={'600'}>기업 정보</Typography>
					<Grid container display={'flex'} gap={1} mt={2}>
						{companyInfoConfig.map((item, index) => {
							return (
								<Grid item key={index} sm={5.9} xs={12}>
									<Typography
										fontWeight={'600'}
										color={'grey'}
										variant="caption"
				...
				{/* 사업 소개 */}
				<Box mt={2}>
					<Typography fontWeight={'600'}>사업 소개</Typography>
					<Grid container display={'flex'} gap={1} mt={2}>
						{businessIntroductionConfig.map((item, index) => {
							return (
								<Grid
									item
									key={index}
					...
					<Box mt={2} display={'flex'}>
						...
						<Box display={'flex'} gap={2} flexWrap={'wrap'}>
							{isEdit ? (
								...
								companyIntroductionImages.map((item, index) => {
									return (
										<Box
											width={'150px'}
											height={'150px'}
				...
				{/* 투자 정보 */}
				<Box mt={2}>
					<Typography fontWeight={'600'}>투자 정보</Typography>
					<Grid container display={'flex'} gap={1} mt={2} mb={2}>
					...
					{isEdit ? (
						<Grid
							...
							position={'relative'}
						>
							{investHistoryConfig.map((item, index) => {
								return (
									<Grid
										item
										key={index}
									...
									>
										...
										<SupportiInput
											...
											value={newInvestInfo[item.value]}
											setValue={(value) => {
												setNewInvestInfo({
													...newInvestInfo,
													[item.value]:
